import { WhitehackDice } from "../dice.js";

export class WhitehackActor extends Actor {
  /**
  ...
  prepareData() {
    ...
    if (game.settings.get("whitehack", "initiative") != "group") {
  ...
  getExperience(value, options = {}) {
    ...
    return this.update({
      ...
      ChatMessage.create({
        content: game.i18n.format("WH.messages.GetExperience", {
          name: this.name,
          value: modified,
        }),
        speaker,
  ...
  generateSave(hd) {
    let saves = {};
    for (let i = 0; i <= hd; i++) {
      let tmp = CONFIG.WH.monster_saves[i];
      if (tmp) {
        saves = tmp;
      }
    }
  ...
  rollSave(save, options = {}) {
    const label = game.i18n.localize(`WH.saves.${save}.long`);
    ...
    const data = {
      ...
      details: game.i18n.format("WH.roll.details.save", { save: label }),
    };

    let skip = options.event && options.event.ctrlKey;

    // Roll and return
    return WhitehackDice.Roll({
      event: options.event,
      parts: rollParts,
      data: data,
      skipDialog: skip,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.format("WH.roll.save", { save: label }),
      title: game.i18n.format("WH.roll.save", { save: label }),
    });
  }
  ...
  rollMorale(options = {}) {
    ...
    return WhitehackDice.Roll({
      ...
      skipDialog: true,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.localize("WH.roll.morale"),
      title: game.i18n.localize("WH.roll.morale"),
    });
  }

  rollLoyalty(options = {}) {
    const label = game.i18n.localize(`WH.roll.loyalty`);
    const rollParts = ["2d6"];
  ...
  rollReaction(options = {}) {
    ...
    const data = {
      ...
      roll: {
        type: "table",
        table: {
          2: game.i18n.format("WH.reaction.Hostile", {
            name: this.data.name,
          }),
          3: game.i18n.format("WH.reaction.Unfriendly", {
            name: this.data.name,
          }),
          6: game.i18n.format("WH.reaction.Neutral", {
            name: this.data.name,
          }),
          9: game.i18n.format("WH.reaction.Indifferent", {
            name: this.data.name,
          }),
          12: game.i18n.format("WH.reaction.Friendly", {
            name: this.data.name,
          }),
        },
      },
    ...
    return WhitehackDice.Roll({
      ...
      skipDialog: skip,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.localize("WH.reaction.check"),
      title: game.i18n.localize("WH.reaction.check"),
    });
  }

  rollCheck(score, options = {}) {
    const label = game.i18n.localize(`WH.scores.${score}.long`);
    const rollParts = ["1d20"];
    ...
    const data = {
      ...
      details: game.i18n.format("WH.roll.details.attribute", {
        score: label,
      }),
    };
    ...
    return WhitehackDice.Roll({
      ...
      skipDialog: skip,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.format("WH.roll.attribute", { attribute: label }),
      title: game.i18n.format("WH.roll.attribute", { attribute: label }),
    });
  }

  rollHitDice(options = {}) {
    const label = game.i18n.localize(`WH.roll.hd`);
  ...
  rollAppearing(options = {}) {
    ...
    return WhitehackDice.Roll({
      ...
      skipDialog: true,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.format("WH.roll.appearing", { type: label }),
      title: game.i18n.format("WH.roll.appearing", { type: label }),
    });
  }

  rollExploration(expl, options = {}) {
    const label = game.i18n.localize(`WH.exploration.${expl}.long`);
    const rollParts = ["1d6"];
    ...
    const data = {
      ...
      details: game.i18n.format("WH.roll.details.exploration", {
        expl: label,
      }),
    };
    ...
    return WhitehackDice.Roll({
      ...
      skipDialog: skip,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.format("WH.roll.exploration", { exploration: label }),
      title: game.i18n.format("WH.roll.exploration", { exploration: label }),
    });
  }
  ...
  rollDamage(attData, options = {}) {
    ...
    WhitehackDice.Roll({
      ...
      skipDialog: true,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: `${attData.label} - ${game.i18n.localize("WH.Damage")}`,
      title: `${attData.label} - ${game.i18n.localize("WH.Damage")}`,
    });
  }
  ...
  rollAttack(attData, options = {}) {
    ...
    const rollParts = ["1d20"];
    const dmgParts = [];
    let label = game.i18n.format("WH.roll.attacks", {
      name: this.data.name,
    });
    if (!attData.item) {
      dmgParts.push("1d6");
    } else {
      label = game.i18n.format("WH.roll.attacksWith", {
        name: attData.item.name,
      });
      dmgParts.push(attData.item.data.damage);
    }

    let ascending = game.settings.get("whitehack", "ascendingAC");
    if (ascending) {
  ...
  computeEncumbrance() {
    ...
    const data = this.data.data;
    let option = game.settings.get("whitehack", "encumbranceOption");
  ...
  _calculateMovement() {
    ...
    if (["detailed", "complete"].includes(option)) {
      ...
      if (weight > game.settings.get("whitehack", "significantTreasure")) {
        data.movement.base -= 30;
      }
    }
  }
  ...
  computeModifiers() {
    ...
    data.scores.str.mod = WhitehackActor._valueFromTable(
      standard,
      data.scores.str.value
    );
    data.scores.int.mod = WhitehackActor._valueFromTable(
      standard,
      data.scores.int.value
    );
    data.scores.dex.mod = WhitehackActor._valueFromTable(
      standard,
      data.scores.dex.value
    );
    data.scores.cha.mod = WhitehackActor._valueFromTable(
      standard,
      data.scores.cha.value
    );
    data.scores.wis.mod = WhitehackActor._valueFromTable(
      standard,
      data.scores.wis.value
    );
    data.scores.con.mod = WhitehackActor._valueFromTable(
      standard,
    ...
    data.scores.dex.init = WhitehackActor._valueFromTable(
      capped,
      data.scores.dex.value
    );
    data.scores.cha.npc = WhitehackActor._valueFromTable(
      capped,
      data.scores.cha.value
    );
    ...
    data.exploration.odMod = WhitehackActor._valueFromTable(
      od,
      data.scores.str.value
    );
    ...
    data.languages.literacy = WhitehackActor._valueFromTable(
      literacy,
      data.scores.int.value
    );
    ...
    data.languages.spoken = WhitehackActor._valueFromTable(
      spoken,
      data.scores.int.value
    );
  }
