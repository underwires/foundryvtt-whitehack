import { WhitehackActor } from "./entity.js";
import { WhitehackActorSheet } from "./actor-sheet.js";
import { WhitehackCharacterModifiers } from "../dialog/character-modifiers.js";
import { WhitehackCharacterCreator } from "../dialog/character-creation.js";

/**
 * Extend the basic ActorSheet with some very simple modifications
 */
export class WhitehackActorSheetCharacter extends WhitehackActorSheet {
  constructor(...args) {
    super(...args);
  }
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["whitehack", "sheet", "actor", "character"],
      template: "systems/whitehack/templates/actors/character-sheet.html",
      width: 450,
      height: 530,
      resizable: true,
      tabs: [
  ...
  generateScores() {
    new WhitehackCharacterCreator(this.actor, {
  ...
  getData() {
    ...
    data.config.ascendingAC = game.settings.get("whitehack", "ascendingAC");
    data.config.initiative = game.settings.get("whitehack", "initiative") != "group";
    data.config.encumbrance = game.settings.get("whitehack", "encumbranceOption");
  ...
  async _chooseLang() {
    let choices = CONFIG.WH.languages;

    let templateData = { choices: choices },
      dlg = await renderTemplate(
        "/systems/whitehack/templates/actors/dialogs/lang-create.html",
        templateData
      );
    ...
    return new Promise((resolve) => {
      new Dialog({
        ...
        buttons: {
          ok: {
            label: game.i18n.localize("WH.Ok"),
          ...
          cancel: {
            ...
            label: game.i18n.localize("WH.Cancel"),
          },
        },
        default: "ok",
      }).render(true);
  ...
  _pushLang(table) {
    ...
    this._chooseLang().then((dialogInput) => {
      const name = CONFIG.WH.languages[dialogInput.choice];
      if (update.value) {
  ...
  _onShowModifiers(event) {
    event.preventDefault();
    new WhitehackCharacterModifiers(this.actor, {
      top: this.position.top + 40,
      left: this.position.left + (this.position.width - 400) / 2,
    }).render(true);
  }
