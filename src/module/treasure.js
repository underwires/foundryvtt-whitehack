export const augmentTable = (table, html, data) => {
  ...
  const flag = table.object.getFlag("whitehack", "treasure");
  const treasure = flag
  ...
  html.find(".toggle-treasure").click((ev) => {
    let isTreasure = table.object.getFlag("whitehack", "treasure");
    table.object.setFlag("whitehack", "treasure", !isTreasure);
  });
  ...
  if (flag) {
    ...
    // Replace Roll button
    const roll = `<button class="roll-treasure" type="button"><i class="fas fa-gem"></i> ${game.i18n.localize('WH.table.treasure.roll')}</button>`;
...
async function rollTreasure(table, options = {}) {
  ...
  let html = await renderTemplate(
    "systems/whitehack/templates/chat/roll-treasure.html",
    templateData
  );

  let chatData = {
    content: html,
    sound: "/systems/whitehack/assets/coins.mp3"
  }
