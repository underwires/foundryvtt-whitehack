export class WhitehackCombat {
  static rollInitiative(combat, data) {
    // Check groups
    data.combatants = [];
    let groups = {};
    combat.data.combatants.forEach((cbt) => {
      groups[cbt.flags.whitehack.group] = { present: true };
    ...
    Object.keys(groups).forEach((group) => {
      ...
      roll.toMessage({
        flavor: game.i18n.format('WH.roll.initiative', { group: CONFIG["WH"].colors[group] }),
      });
    ...
    for (let i = 0; i < data.combatants.length; ++i) {
      ...
      if (data.combatants[i].actor.data.data.isSlow) {
        ...
        data.combatants[i].initiative =
          groups[data.combatants[i].flags.whitehack.group].initiative;
      }
    }
  }
  ...
  static async individualInitiative(combat, data) {
    ...
    combat.data.combatants.forEach((c, i) => {
      ...
      let messageData = mergeObject({
        ...
        flavor: game.i18n.format('WH.roll.individualInit', { name: c.token.name })
      }, {});
  ...
  static format(object, html, user) {
    ...
    let init = game.settings.get("whitehack", "initiative") == "group";
    if (!init) {
      return;
    }
    ...
    html.find(".combatant").each((_, ct) => {
      ...
      let color = cmbtant.flags.whitehack.group;

      // Append colored flag
      let controls = $(ct).find(".combatant-controls");
      controls.prepend(
        `<a class='combatant-control flag' style='color:${color}' title="${CONFIG.WH.colors[color]}"><i class='fas fa-flag'></i></a>`
      );
    });
    WhitehackCombat.addListeners(html);
  }
    ...
    if (data.initiative && init == "group") {
      ...
      combat.combatants.forEach((ct) => {
        if (
          ...
          ct._id != data._id &&
          ct.flags.whitehack.group == combatant.flags.whitehack.group
        ) {
  ...
  static addListeners(html) {
    ...
    html.find(".combatant-control.flag").click((ev) => {
      ...
      let colors = Object.keys(CONFIG.WH.colors);
      ...
      game.combat.updateCombatant({
        _id: id,
        flags: { whitehack: { group: colors[index] } },
      });
    });
    ...
    html.find('.combat-control[data-control="reroll"]').click((ev) => {
      ...
      let data = {};
      WhitehackCombat.rollInitiative(game.combat, data);
