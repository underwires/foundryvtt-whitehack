import { WhitehackActor } from '../actor/entity.js';
import { WhitehackDice } from "../dice.js";

export class WhitehackCharacterCreator extends FormApplication {
  static get defaultOptions() {
    const options = super.defaultOptions;
    options.classes = ["whitehack", "dialog", "creator"],
      options.id = 'character-creator';
    options.template =
      'systems/whitehack/templates/actors/dialogs/character-creation.html';
    options.width = 235;
    return options;
  }
   ...
   */
  get title() {
    return `${this.object.name}: ${game.i18n.localize('WH.dialog.generator')}`;
  }
  ...
  rollScore(score, options = {}) {
    ...
    const label = score != "gold" ? game.i18n.localize(`WH.scores.${score}.long`) : "Gold";
    const rollParts = ["3d6"];
    const data = {
      roll: {
        type: "result"
    ...
    return WhitehackDice.Roll({
      ...
      flavor: game.i18n.format('WH.dialog.generateScore', { score: label, count: this.object.data.counters[score] }),
      title: game.i18n.format('WH.dialog.generateScore', { score: label, count: this.object.data.counters[score] }),
    });
  }
  ...
  async close() {
    ...
    const templateData = {
      config: CONFIG.WH,
      scores: scores,
      title: game.i18n.localize("WH.dialog.generator"),
      stats: this.object.data.stats,
      gold: gold
    }
    const content = await renderTemplate("/systems/whitehack/templates/chat/roll-creation.html", templateData)
    ChatMessage.create({
      content: content,
      speaker,
    });
  ...
  async _onSubmit(event, { updateData = null, preventClose = false, preventRender = false } = {}) {
    ...
    const itemData = {
      name: "GP",
      type: "item",
      img: "/systems/whitehack/assets/gold.png",
      data: {
        treasure: true,
        cost: 1,
        weight: 1,
